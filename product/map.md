...
- **10 score-bucket colors (orange-red→green), soft decile ramp.** Pin sprite, result-list rank pill, and dot all derive color from one source of truth — the palette in `apps/mobile/src/constants/score-bucket-palette.json` (read by `apps/mobile/src/utils/quality-color.ts`): 10 deciles over the native 0–10 score, `tier = clamp(floor(score), 0, 9)` (9–10 green … 0–1 orange-red). The ramp is a SOFT, perceptually OKLab-spaced red→green with a chroma-lifted gold→green middle; deciles are equal-population (the global percentile puts ~10% per tier) so the map stays evenly colored across the full range. A future colorblind setting swaps in the viridis ramp (staged in the same json under `colorblind`, no runtime toggle yet). Color carries fine-grained objective-quality signal at a glance.
- **In-viewport RANK badge vs out-of-viewport SCORE badge.** Inside the submitted-search viewport a pin shows its frozen search RANK (1..99, contextual); pan outside that viewport and it shows the intrinsic 0–10 SCORE (stable as you move away, 1 decimal). Ranks above 99 fold to a shared "99+" overflow sprite. The number is baked into the pin sprite so pin and number order together as one unit.
- **Active/selected pin recolor.** Tapping a marker swaps its pin to an active-color sprite (#ff3368) keeping the rank number, and force-promotes it so a tapped restaurant ranked below 30 doesn't demote to a dot on pan.
...
Smooth pin/dot transitions across pan and zoom are the difference between a premium map and a janky one, so the map carries a large LOD/rendering subsystem with its own telemetry harness. The product invariants the substrate must hold:
...
- **Viewport bounds ARE the location filter.** Each search query carries the visible NE/SW bounds; the DB filters restaurants within them (spatial index) before ranking.
- **Open Now + price filter.** A binary open-now filter (current time vs stored hours) and a price-level filter apply before ranking — the free power-filters that pair with the map.
...
- **Dense-urban / low-zoom declutter.** When zoomed far out, markers collapse to a central blob and the outer screen goes empty. The fix is some combination of clustering (count bubbles that expand on zoom-in) and a score-weighted **heat / density view** (an alternate mode showing where the high-Crave-Score clusters sit in a city — good for "which neighborhood should I go to").
