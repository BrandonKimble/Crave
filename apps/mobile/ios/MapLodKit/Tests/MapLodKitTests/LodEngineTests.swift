...
final class LodEngineTests: XCTestCase {
  ...
  /// Run step() to a fixed point (or fail). Returns ticks taken.
  @discardableResult
  private func settle(_ e: inout LodEngine, maxTicks: Int = 1000, dt: Double = 1.0 / 60.0) -> Int {
    var ticks = 0
    while !e.isIdle && ticks < maxTicks { e.step(dtSeconds: dt); ticks += 1 }
  ...
  // MARK: FM#4 — no snapping (per-anchor eased integration, bounded per tick)

  func testNeverSnapsToTargetInOneTick() {
    var e = LodEngine(budget: 1, fadeSeconds: 0.18)
    e.setRanking(ranking(1))
    e.decide(onScreenKeys: keys([1]))
    let writes = e.step(dtSeconds: 1.0 / 60.0)   // one ~16ms tick of a 180ms fade
    XCTAssertEqual(writes.count, 1)
    let op = writes[0].pinOpacity
    XCTAssertGreaterThan(op, 0.0)
    XCTAssertLessThan(op, 0.5, "a single tick of a 180ms fade must NOT jump near full — no snap")
  }
  ...
  func testCrossfadeTerminatesExactlyAtTarget() {
    ...
    e.decide(onScreenKeys: keys([1]))
    let ticks = settle(&e)
    XCTAssertEqual(e.pinOpacity("r1"), 1.0, "ε-snap drives opacity to exactly 1")
  ...
  // MARK: interrupt reversal (no snap when want flips mid-fade)

  func testInterruptedFadeReversesSmoothlyFromCurrent() {
    var e = LodEngine(budget: 1, fadeSeconds: 0.18)
    e.setRanking(ranking(2))
    e.decide(onScreenKeys: keys([1]))           // r1 promoting
    e.step(dtSeconds: 1.0 / 60.0); e.step(dtSeconds: 1.0 / 60.0)
    ...
    let writes = e.step(dtSeconds: 1.0 / 60.0)
    let after = writes.first { $0.markerKey == "r1" }!.pinOpacity
    XCTAssertLessThan(after, mid, "reverses DOWN from its current value")
  ...
  func testDemotedAnchorConvergesToZeroEvenOffScreen() {
    ...
    e.setRanking(ranking(2))
    e.decide(onScreenKeys: keys([1])); settle(&e)
  ...
  func testReturnToOriginRestoresExactTopN() {
    ...
    let (p2, _) = e.decide(onScreenKeys: origin); settle(&e)
  ...
  func testIdleEngineProducesNoWrites() {
    ...
    e.decide(onScreenKeys: keys([1, 2])); settle(&e)
    XCTAssertEqual(e.step(dtSeconds: 1.0 / 60.0).count, 0, "a settled engine writes nothing (no idle churn)")
  }
}
