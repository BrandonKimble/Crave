...
//
// THE ARRAY IS THE TYPE (F908). `ResidencyManagedSceneKey` used to be declared
...
export const RESIDENCY_MANAGED_SCENES = [
...
/** The residency-managed key space: DERIVED from the membership array above, never
 *  restated. (The `satisfies` on the array keeps every entry a real OverlayKey.) */
export type ResidencyManagedSceneKey = (typeof RESIDENCY_MANAGED_SCENES)[number];
 ...
 *  to ONE shared singleton resident unit instead of per-identity units, which is the
 ...
 *  `Record<ResidencyManagedSceneKey, ...>` (whose key type is itself derived from the
 *  membership array, F908), joining the residency set now FORCES the scene to declare
 ...
 *  Record<ResidencyManagedSceneKey, ResidentUnitIdentityResolver>". */
const RESIDENT_UNIT_IDENTITY_BY_SCENE: Record<
  ResidencyManagedSceneKey,
  ResidentUnitIdentityResolver
> = {
...
export const residentUnitIdentityOf = (entry: OverlayRouteEntry): string | null => {
  ...
  const resolve = isResidencyManagedScene(entry.key)
    ? RESIDENT_UNIT_IDENTITY_BY_SCENE[entry.key]
    : SINGLETON_UNIT;
...
//
// F909: this used to be `[...RESIDENCY_MANAGED_SCENES, 'search']` under a paragraph
// claiming "search joins HERE without joining the residency set" — but 'search' is
// already the LAST entry of the residency set, so the spread listed it twice and the
// paragraph was simply false beside the code. Search DOES join the residency set (its
...
// not as a re-listing.
export const DEFERRED_PUBLICATION_SCENES: readonly OverlayKey[] = RESIDENCY_MANAGED_SCENES;
