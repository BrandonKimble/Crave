...
const PAGINATION_TRACE_THROTTLE_MS = 800;
...
export const useSearchRootResultsSheetLoadMoreRuntime = ({
  ...
  const lastPaginationTraceAtMsRef = React.useRef<number | null>(null);
  ...
  const markResultsListUserScrollStart = React.useCallback(() => {
    if (shouldLogSearchStateChanges && !hasUserScrolledResultsRef.current) {
      logger.debug('[SearchPerf] scrollStart marked');
    }
  ...
  const handleResultsEndReached = React.useCallback(() => {
    if (shouldLogSearchStateChanges) {
      logger.debug(
        `[SearchPerf] endReached scrolled=${hasUserScrolledResultsRef.current} allow=${allowLoadMoreForCurrentScrollRef.current} canLoadMore=${canLoadMoreRef.current} loading=${isSearchLoadingRef.current} loadingMore=${isLoadingMoreRef.current} page=${currentPageRef.current}`
      );
    }
  ...
  const handleResultsListUserScrollActivity = React.useCallback(
    (offsetY: number, distanceFromEnd: number) => {
      if (shouldLogSearchStateChanges) {
        ...
        const lastTraceAtMs = lastPaginationTraceAtMsRef.current;
        if (lastTraceAtMs == null || now - lastTraceAtMs > PAGINATION_TRACE_THROTTLE_MS) {
          lastPaginationTraceAtMsRef.current = now;
          logger.debug(
            `[SearchPerf] trace offsetY=${Math.round(offsetY)} distanceFromEnd=${Math.round(distanceFromEnd)}`
          );
        }
      }
      if (offsetY >= USER_SCROLL_ACTIVITY_MIN_OFFSET_PX) {
        if (shouldLogSearchStateChanges && !hasUserScrolledResultsRef.current) {
          logger.debug(`[SearchPerf] scroll activity marked offsetY=${Math.round(offsetY)}`);
        }
        hasUserScrolledResultsRef.current = true;
        allowLoadMoreForCurrentScrollRef.current = true;
      }
    ...
    [handleResultsEndReached, shouldLogSearchStateChanges]
  );
