...
const PUSHSHIFT_BASE_DIR = path.join(__dirname, '..', 'data', 'pushshift', 'archives');
...
async function validateArchiveIntegrity(): Promise<void> {
  ...
  for (const subreddit of SUBREDDITS) {
    ...
    for (const fileType of FILE_TYPES) {
      ...
      try {
        ...
        const { stdout } = await execAsync(`zstd -dc "${filePath}" | head -n 3`);
        ...
        for (const line of lines) {
          if (line.trim()) {
            try {
              ...
              if (typeof parsed === 'object' && (parsed.id || parsed.name)) {
                validJsonLines++;
              }
            } catch (jsonError) {
              console.log(`   ⚠️  Invalid JSON line: ${line.substring(0, 100)}...`);
            }
          }
        }

        const isValidNdjson = validJsonLines > 0;
        console.log(`   ✅ ${fileName} - Valid ndjson format (${validJsonLines}/${lines.length} valid lines)`);
  ...
  console.log(`   Total sample lines validated: ${validFiles.reduce((sum, r) => sum + r.sampleLineCount, 0)}`);
  
  if (validFiles.length === results.length) {
    console.log(`   🎉 All archive files have valid zstd compression and ndjson format!`);
    process.exit(0);
  } else {
    console.error(`   ⚠️  ${results.length - validFiles.length} files failed validation`);
