...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!execute) {
      console.log(
        `[entity-names] dry-run — ${renames.length} renames available`,
      );
      return;
    }
    ...
    // EVERY path closes the context (red team 2026-08-04). This close used to
    // sit inside the dry-run branch only, so `--execute` printed its summary
    // and then HUNG forever — an operator ctrl-C'ing a rename pass had no way
    // to know whether it had finished. Closing also flushes the usage ledger.
    await app.close();
  }
}
