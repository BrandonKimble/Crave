...
async function conflicts(): Promise<void> {
  ...
  >`
    ...
    HAVING COUNT(DISTINCT type) >= 2
       AND NOT (array_agg(DISTINCT type ORDER BY type) = ARRAY['item','ingredient'])
    ORDER BY name
  `;
  console.log(
...
async function junk(): Promise<void> {
  const genericPlaces = await prisma.$queryRaw<
    { name: string; entity_id: string }[]
  >`
    SELECT e.name, e.entity_id::text
    FROM core_entities e
    WHERE e.status = 'active' AND e.type = 'place'
  ...
  `;
  const genericItems = await prisma.$queryRaw<
    { name: string; type: string; entity_id: string }[]
  >`
    SELECT e.name, e.type::text, e.entity_id::text
    FROM core_entities e
    WHERE e.status = 'active' AND e.type IN ('item','item_attribute')
  ...
  >`
    ...
    FROM core_entities e
    WHERE e.status = 'active'
      AND e.type IN ('item','item_attribute','place_attribute')
      AND array_length(string_to_array(TRIM(e.name), ' '), 1) >= 5
    ORDER BY e.name
  `;
  console.log(`zero-location generic restaurants: ${genericPlaces.length}`);
  genericPlaces.forEach((r) => console.log(`  ${r.name}\t${r.entity_id}`));
  console.log(`generic food-side words: ${genericItems.length}`);
  genericItems.forEach((r) => console.log(`  ${r.type}\t${r.name}`));
