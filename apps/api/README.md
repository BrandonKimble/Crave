...
# Code quality
yarn workspace api lint              # ESLint with auto-fix
yarn workspace api type-check        # TypeScript check
yarn workspace api format            # Prettier formatting
```
...
```bash
# Migrations
yarn workspace api prisma:migrate       # Run migrations
yarn workspace api db:migrate:deploy    # Production deployment
yarn workspace api db:migrate:reset     # Reset database (dev only)
yarn workspace api db:migrate:status    # Check migration status
...
yarn workspace api docker:down          # Stop services
```
...
# Test specific files
yarn workspace api test entity.repository.spec.ts
yarn workspace api test --testNamePattern="create entity"
```
...
yarn workspace api prisma:migrate status
...
# Clean test data
yarn workspace api test -- --clearCache
```
