...
const EXEMPT: Record<string, string> = {
  ...
  'entity_alias.entity_id':
    'OWNED per-entity vocabulary (ON DELETE CASCADE), not a reference to someone else’s entity — the rows live and die with the entity the anchor already protects, and hold no user link',
  'entity_labels.entity_id':
    'OWNED per-entity vocabulary (ON DELETE CASCADE) — same lifetime as entity_alias; labels are generated by the M2 sweep and regenerate, they anchor nothing',
};
