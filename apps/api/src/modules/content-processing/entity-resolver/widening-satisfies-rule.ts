...
export function buildAttributeSatisfiesPrompt(
  ...
  return [
    ...
    `Doubt says "reject": a wrong admission poisons the filter for every`,
    `searcher; a miss only narrows it. For each case give a one-clause`,
    `reason grounded in what the searcher would experience — never the bare`,
...
export function buildIngredientSatisfiesPrompt(
  ...
  return [
    ...
    ``,
    `Judge THIS direction only — never assume the reverse also holds.`,
    `Doubt says "reject": a wrong admission puts the wrong dish in front of`,
