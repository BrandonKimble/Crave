 ...
 * leg that kills each:
 *  - /markets/resolve + /markets/resolve-ip + IpLocationService: mobile
 *    launch intents (IP→metro map fallback, viewport resolve for pre-cut
 *    builds) — dies with the mobile HOME-PLACE REGISTRATION leg
 *    (registration → placeAt → homePlaceId).
 ...
 *  - SignalsService.bboxFromMarketKey + the polls legacy feed arm +
 *    NotificationsService quarantined market fallback: legacy market-keyed
 *    poll rows — die with the LEGACY-POLL-EXPIRY leg.
