...
export class LLMConcurrentProcessingService implements OnModuleInit {
  ...
  async processConcurrent(
    ...
    const averageChunkTime = allDurations.length > 0 ? allDurations.reduce((sum, d) => sum + d, 0) / allDurations.length : 0;
    const fastestChunk = allDurations.length > 0 ? Math.min(...allDurations) : 0;
    const slowestChunk = allDurations.length > 0 ? Math.max(...allDurations) : 0;
    ...
    this.logger.info('Concurrent processing completed', {
      ...
      fastestChunk,
      slowestChunk,
      totalMentions: successfulResults.reduce((sum, r) => sum + r.mentions.length, 0)
    });
   ...
   */
  private validateOutputStructure(output: LLMOutputStructure, chunkId: string): void {
    if (!output || !output.mentions) {
      throw new Error(`Invalid output structure for chunk ${chunkId}: missing mentions array`);
    }

    for (const mention of output.mentions) {
      // Check vital fields
      if (mention.dish_primary_category && mention.dish_is_menu_item === undefined) {
      ...
      if (mention.general_praise === undefined) {
        throw new Error(`Missing vital field: general_praise in chunk ${chunkId}, mention ${mention.temp_id}`);
      }

      if (!mention.restaurant_temp_id) {
        throw new Error(`Missing vital field: restaurant_temp_id in chunk ${chunkId}, mention ${mention.temp_id}`);
      }

      if (!mention.source_id) {
        throw new Error(`Missing vital field: source_id in chunk ${chunkId}, mention ${mention.temp_id}`);
      }
    }
  }
