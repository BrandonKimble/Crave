...
export class SmartLLMProcessor implements OnModuleInit {
  ...
  constructor(
    ...
    @Inject(CentralizedRateLimiter) private readonly centralizedRateLimiter: CentralizedRateLimiter,
  ) {}
  ...
  onModuleInit() {
    ...
    this.logger.info('Bulletproof LLM Processor initialized with reservation-based rate limiting', {
  ...
  async processContent(input: any, llmService: LLMService, workerId?: string): Promise<any> {
    ...
    try {
      ...
      const reservation = await this.rateLimiter.reserveRequestSlot(effectiveWorkerId);
      
      // 2. Wait until the reserved time if necessary
      if (reservation.waitMs > 0) {
        this.logger.debug(`Worker ${effectiveWorkerId} waiting ${reservation.waitMs}ms for reserved slot`, {
          correlationId: CorrelationUtils.getCorrelationId(),
      ...
      await this.rateLimiter.confirmReservation(effectiveWorkerId, reservation.reservationTime);
  ...
  async getMetrics(): Promise<RateLimitMetrics> {
    ...
    const avgWaitTime = this.processedRequests > 0 
      ? Math.round(this.totalWaitTime / this.processedRequests)
      : 0;
    
    const zeroWaitPercent = this.processedRequests > 0
    ...
    return {
      ...
      performance: {
        ...
        zeroWaitPercent,
        reservationAccuracy: rateLimiterMetrics.reservations?.avgAccuracyMs || 0
      }
    };
  }
