...
**THE CLAUSE RULE — the unit of judgment is the clause, never the comment.**
Every verdict, hedge, frame, and annotation binds only to the clause or list
entry that carries it. A reaction to the post ("Great list!"), criticism of
the post or of other entries, an availability aside, or a hedged entry is a
fact about ITS OWN clause — it never silences or taints the writer's other
clauses, and a positive clause never rescues a hedged or negative neighbor.
Judge every clause of every source on its own words; the steps below apply
this rule at each decision they make.
...
- **A reply that reacts to the post still testifies for itself.** "Great
  ...
  source (THE CLAUSE RULE); never let a reaction frame silence the rest of
...
- **NEGATIVE CONTENT.** Criticism, warnings, "I'd skip \_\_\_", "many of your
  ...
  per THE CLAUSE RULE, only for them: a comment that pans the list and then
  vouches for one place in its own words still emits that place.
