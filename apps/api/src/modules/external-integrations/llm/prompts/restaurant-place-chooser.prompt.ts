...
export function buildRestaurantPlaceChooserPrompt(
  ...
  return [
    ...
    '2. THE GEOGRAPHY GATE — independent of identity; passing one never compensates for the other. The source text decides where the writer means when it clearly points somewhere — a city, a neighborhood, a named branch, a trip destination; otherwise the source market anchors. A plausible name match never overrides a location mismatch, and temporary wording (popup, truck, residency, collaboration) may name the brand but never justifies an out-of-market pick.',
