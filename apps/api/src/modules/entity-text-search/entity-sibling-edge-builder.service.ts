 ...
 */
@Injectable()
export class EntitySiblingEdgeBuilderService extends DerivedIndexJob {
  protected readonly logger: LoggerService;
  protected readonly derivedTable = 'derived_entity_sibling_edges';
  protected readonly disableFlagEnv = 'ENTITY_SIBLING_EDGES_REBUILD_ENABLED';
  protected readonly alert = {
    kind: 'sibling_edges_empty',
    title: 'Dense sibling widening is silently disabled',
    consequence: '"Similar dishes" widening is OFF for every search.',
  };
  ...
  protected async rebuild(): Promise<{ input: number; output: number }> {
    const { anchors, edges } = await this.rebuildAll();
    return { input: anchors, output: edges };
  }
