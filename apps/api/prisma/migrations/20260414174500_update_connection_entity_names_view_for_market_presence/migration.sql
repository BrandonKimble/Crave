...
SELECT
  ...
  COALESCE(
    restaurant_market_presence.restaurant_market_keys,
    ARRAY[]::text[]::character varying[]
  ) AS restaurant_market_keys,
  r.restaurant_attributes AS restaurant_attribute_ids,
  COALESCE(
    restaurant_attribute_names.restaurant_attribute_names,
    ARRAY[]::text[]::character varying[]
  ...
  COALESCE(category_names.category_names, ARRAY[]::text[]::character varying[])
    AS category_names,
  c.food_attributes AS food_attribute_ids,
  COALESCE(
    food_attribute_names.food_attribute_names,
    ARRAY[]::text[]::character varying[]
...
LEFT JOIN LATERAL (
  SELECT
    array_agg(emp.market_key ORDER BY emp.market_key) FILTER (
      WHERE emp.market_key IS NOT NULL
    ) AS restaurant_market_keys
  FROM public.core_entity_market_presence emp
  WHERE emp.entity_id = c.restaurant_id
) restaurant_market_presence ON TRUE
LEFT JOIN LATERAL (
  SELECT
    array_agg(e.name ORDER BY u.ordinality) FILTER (WHERE e.name IS NOT NULL)
      AS category_names
