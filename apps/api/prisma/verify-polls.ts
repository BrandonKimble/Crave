...
async function main() {
  ...
  for (const p of polls) {
    ...
    if (p.topic.targetDishId) {
      const dish = await prisma.entity.findUnique({
        where: { entityId: p.topic.targetDishId },
      });
      console.log(` -> Linked Dish: ${dish?.name} (${dish?.type})`);
    }
    if (p.topic.targetRestaurantId) {
      const rest = await prisma.entity.findUnique({
        where: { entityId: p.topic.targetRestaurantId },
      });
    ...
    for (const o of p.options) {
      ...
      if (o.restaurantId) {
        const r = await prisma.entity.findUnique({
          where: { entityId: o.restaurantId },
        });
        linkedDetails = `Rest: ${r?.name}`;
      } else if (o.foodId) {
        const f = await prisma.entity.findUnique({
          where: { entityId: o.foodId },
        });
