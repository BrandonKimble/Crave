...
- `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-root-request-lane-runtime.ts`
- `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-root-action-lanes-runtime.ts`
- `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-restaurant-route-publication-runtime.ts`
...
- `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/profile/profile-owner-runtime.ts`
- `/Users/brandonkimble/crave-search/apps/mobile/src/overlays/RestaurantRouteLayerHost.tsx`
...
- `/Users/brandonkimble/crave-search/apps/mobile/ios/cravesearch/UIFrameSamplerBridge.m`
...
## 0.1 Current Repo Reality Check (2026-04-10)
...
- `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/index.tsx` is no longer the original ~10k LOC root monolith or the earlier ~2.3k LOC composition layer. It is now a thin root entrypoint, while `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-root-runtime.ts` owns the real Search root composition.
- The Search root has moved again since the earlier audit passes: `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-root-runtime.ts` is now down to roughly 1.3k LOC and delegates major seams into explicit shared owners for scaffold/runtime primitives, request lane assembly, action-lane composition, Search-origin restaurant route publication, and the final map/overlay render-surface assembly:
  - `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-root-scaffold-runtime.ts`
  - `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-root-request-lane-runtime.ts`
  - `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-root-action-lanes-runtime.ts`
  - `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-restaurant-route-publication-runtime.ts`
  - `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-root-map-render-surface-runtime.ts`
  - `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-root-overlay-render-surface-runtime.ts`
    The root is still the top-level Search orchestrator, but it is no longer the same all-inline 3.8k owner described by older reviews, and it no longer owns both render surfaces inline either.
- The Search root now has explicit top-level root contracts:
  ...
  - `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/search-root-core-runtime-contract.ts`
    Those two files now define the top-level stop line for the surviving root-side owners instead of repeating `Pick<UseSearchRootRuntimeArgs, ...>` or another repeated five-runtime handoff bag.
...
- The restaurant route host is flatter than older plan text suggests. The live route now resolves through:
  - `/Users/brandonkimble/crave-search/apps/mobile/src/overlays/RestaurantRouteLayerHost.tsx`
  - `/Users/brandonkimble/crave-search/apps/mobile/src/overlays/useResolvedRestaurantRouteHostModel.ts`
  - `/Users/brandonkimble/crave-search/apps/mobile/src/overlays/OverlaySheetShell.tsx`
    The current remaining bridge edge is narrower than the older `snapshotJSON` path: JS now passes a structured `snapshot` payload through `/Users/brandonkimble/crave-search/apps/mobile/src/overlays/RestaurantPanelSnapshotNativeView.tsx`, Swift quarantines the raw bridge object at the property boundary and decodes it once with explicit validation in `/Users/brandonkimble/crave-search/apps/mobile/ios/cravesearch/CraveRestaurantPanelSnapshotView.swift`, Android now rejects malformed payloads through the strict decoder in `/Users/brandonkimble/crave-search/apps/mobile/android/app/src/main/java/com/crave/RestaurantPanelSnapshotPayload.java`, and the last iOS seam is the extern prop export in `/Users/brandonkimble/crave-search/apps/mobile/ios/cravesearch/UIFrameSamplerBridge.m`, which still declares the bridge prop as `NSDictionary`. Under the current legacy React Native bridge, that remaining seam is mostly a container-typing artifact rather than a missing owner boundary, so further tightening should be justified by a real native-bridge migration or measured behavior need instead of purity alone.
- Results overlay/sheet/chrome/root screen state has been split into dedicated Search hooks and presentation components, including:
  ...
  - inline shortcut-harness ref publication in `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-root-runtime.ts`
  - `/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/runtime/shared/use-search-results-panel-data-runtime.tsx`
...
- The restaurant route render boundary is narrower too: `AppOverlayRouteHost` no longer assembles separate global-vs-Search restaurant overlay props inline, and a dedicated `RestaurantRouteLayerHost` now owns the route-level restaurant host resolution before delegating into the shared sheet shell.
...
- The restaurant route host-model boundary is narrower too: `RestaurantRouteLayerHost` no longer wires raw bottom-sheet presentation/snap primitives directly into an older dedicated restaurant host wrapper, and now resolves one dedicated `RestaurantRouteHostModel` through `restaurantRouteHostContract.ts` plus `useResolvedRestaurantRouteHostModel.ts` before mounting the route host.
- The restaurant host/runtime split is narrower too: `RestaurantRouteLayerHost.tsx` now stays on restaurant route/content ownership while the shared native sheet/scroll/motion runtime stays under `OverlaySheetShell.tsx`; the visible restaurant lane no longer depends on the deleted `RestaurantOverlayHost.tsx` / `RestaurantSheetHost.tsx` wrapper pair.
- The restaurant native payload boundary is stronger too: the typed JS payload now crosses the bridge as a structured `snapshot` object prop instead of a JSON string or ad hoc container reads. `CraveRestaurantPanelSnapshotView.swift` now decodes that payload on iOS, `RestaurantPanelSnapshotPayload.java` now decodes the same payload contract on Android, and `CraveRestaurantPanelSnapshotView.swift` plus `RestaurantPanelSnapshotView.java` now render and emit actions from explicit native payload models instead of inline bridge-container reads.
...
- The profile transition-record boundary is narrower too: the old profile controller lane no longer mutates `preparedSnapshot` / `dismissHandled` / `profileOpenSettleState` directly for open commit, close commit, seed-time dismiss reset, or close-time reset, and now delegates that transition-record mutation batch to controller-owned apply/reset helpers in `profile-presentation-controller.ts`.
...
- The foreground chrome model is narrower too: the deleted foreground wrappers, route-panel wrapper pair, Search-only docked-polls publication shim, and deleted `use-search-restaurant-route-owner.ts` no longer mix Search-route publication with chrome render-model assembly or keep a one-off restaurant publish seam beside the shared runtime lane. `runtime/shared/use-search-route-panel-publication-runtime.tsx` now owns the full Search-route publication lane over `use-search-results-panel-data-runtime.tsx`, `use-search-results-panel-read-model-runtime.tsx`, `use-search-results-panel-render-policy-runtime.tsx`, `use-search-results-panel-covered-render-runtime.tsx`, `use-search-results-panel-surface-state-runtime.tsx`, `use-search-results-panel-interaction-frost-runtime.tsx`, `use-search-results-panel-surface-background-runtime.tsx`, `use-search-results-panel-surface-overlay-runtime.tsx`, `use-search-results-panel-spec-runtime.tsx`, `use-search-results-panel-route-visibility-runtime.tsx`, and `useSearchRouteOverlayRuntime(...)`, where the overlay-owned route host lane is now split more honestly: `useSearchRouteOverlayPublishedState.ts` now owns the published visual/search-panel/render-policy selectors, `useSearchRouteOverlaySheetKeys.ts` now owns route-sheet key derivation, the deleted `useSearchRouteOverlayPanelSpecFamilies.ts` wrapper no longer sits above the direct family-spec lane, `useResolvedSearchRouteHostModel.ts` now composes the direct polls/poll-creation/bookmarks/profile/save-list specs itself, the deleted `useSearchRouteOverlayPanelSpecsRuntime.ts` wrapper no longer sits above that lane, `useSearchRouteOverlayActiveSheetSpec.ts` now owns active sheet/spec plus final search-interaction resolution, `useSearchRouteOverlaySheetVisibilityState.ts` now owns spec suppression and final overlay visibility policy, `useSearchRouteOverlayRuntimePublication.ts` now owns runtime-store publication of direct `searchPanelSpec` + direct `searchPanelInteractionRef` + direct `dockedPollsPanelInputs` + `renderPolicy`, and `useSearchRouteOverlayRuntime.ts` now stays on thin composition over those lower overlay owners. `index.tsx` now consumes that lower route-publication owner before the root-owned chrome render-model lane in `use-search-root-runtime.ts`. That Search-owned foreground input lane is flatter too: `use-search-results-panel-visual-runtime-model.ts` now owns the final results-panel visual model, while `use-search-root-runtime.ts` now assembles the final suggestion/header/filter input bags inline over the real root suggestion/scaffold/request/visual/action owners and direct filter/search-state reads; the deleted `use-search-root-foreground-suggestion-inputs-runtime.ts`, deleted `use-search-root-foreground-header-inputs-runtime.ts`, deleted `use-search-root-foreground-filters-warmup-inputs-runtime.ts`, deleted `use-search-foreground-suggestion-inputs.ts`, deleted `use-search-foreground-header-inputs.ts`, deleted `use-search-foreground-filters-warmup-inputs.ts`, and the later deleted one-consumer suggestion/header input helper family no longer sit between those owned input lanes and the Search-root foreground input owner, so the root no longer builds those grouped bags inline before handing them to the lower route/render owners. The remaining Search-root foreground interaction/input and visual lane is flatter too: the one-consumer `use-search-suggestion-interaction-runtime.ts` wrapper is deleted, `use-search-root-runtime.ts` now keeps restaurant-selection, profile analytics, camera-transition ports, execution args, pending-marker cleanup, and suggestion-interaction wiring inline at the real root boundary before the final `useSuggestionInteractionController(...)` plus `useProfileOwner(...)` composition for the profile owner and suggestion/header surfaces, `use-search-foreground-interaction-runtime-contract.ts` now owns the shared foreground interaction vocabulary together with grouped `launchIntentArgs`, `submitRuntimeArgs`, `retryRuntimeArgs`, `editingRuntimeArgs`, `overlayRuntimeArgs`, `effectsRuntimeArgs`, and `restaurantOnlyResolutionArgs` lanes, and `use-search-root-runtime.ts` now owns that grouped foreground interaction arg assembly plus the final `use-search-foreground-interaction-runtime.ts` composition directly instead of another one-consumer domain wrapper. `use-search-foreground-launch-intent-runtime.ts` now owns launch-intent routing, `use-search-foreground-submit-runtime.ts` now owns submit/search-this-area/suggestion/recent selection orchestration, `use-search-foreground-retry-runtime.ts` now owns reconnect retry policy, `use-search-foreground-editing-runtime.ts` now owns clear/focus/blur/back editing behavior, `use-search-foreground-overlay-runtime.ts` now owns route-intent replay plus view-more/overlay selection, and `use-search-foreground-interaction-runtime.ts` now stays on thin composition over those lower grouped foreground interaction owners. `use-search-request-status-runtime.ts` now owns grouped search-request plus system-status reads, `use-search-history-runtime.ts` now owns the grouped recent-search / recently-viewed history lane, `use-search-filter-state-runtime.ts` now owns the grouped filter store selection lane, `use-search-root-filter-domain-runtime.ts` now owns the lower `useSearchFilterModalOwner(...)` composition plus the later filter-modal/control surface directly, `use-search-autocomplete-runtime.ts` now owns autocomplete cache/suppression/request lifecycle, `use-search-recent-activity-runtime.ts` now owns recent-search upsert plus recently-viewed buffering/flush policy, `use-search-root-runtime.ts` now owns query/focus/press-in/change handling plus shortcut query reseed behavior inline, `use-search-suggestion-transition-timing-runtime.ts` now owns keyboard-aware transition timing policy, `use-search-suggestion-transition-presence-runtime.ts` now owns transition-driver presence/overlay visibility, `use-search-suggestion-layout-warmth-runtime.ts` now owns layout warmth plus drive-layout policy, `use-search-suggestion-transition-runtime.ts` now stays on thin composition over those lower transition owners, `use-search-suggestion-display-runtime.ts` now owns live suggestion/recent/autocomplete display derivation, `use-search-suggestion-hold-state-runtime.ts` now owns hold snapshot state plus capture/reset primitives, `use-search-suggestion-hold-actions-runtime.ts` now owns submit/close hold command construction, `use-search-suggestion-hold-sync-runtime.ts` now owns hold registration plus query/layout cleanup effects, `use-search-suggestion-held-display-runtime.ts` now owns held suggestion surface outputs, `use-search-suggestion-hold-effects-runtime.ts` now stays on thin composition over those lower hold-lifecycle owners, `use-search-suggestion-hold-runtime.ts` now stays on thin composition over those lower hold owners, `use-search-suggestion-visibility-runtime.ts` now stays on thin composition over those lower visibility owners, `use-search-suggestion-layout-state-runtime.ts`, `use-search-suggestion-layout-visual-runtime.ts`, and `use-search-suggestion-header-holes-runtime.ts` now own the paired layout caches, cutout geometry, and header spacing lane under the thin `use-search-suggestion-layout-runtime.ts` surface, `use-search-root-runtime.ts` now composes that lower suggestion visibility + layout lane directly, `use-search-foreground-visual-runtime.ts` now owns the later chrome/search-shortcut/search-this-area/results-sheet visual lane, `use-search-root-runtime.ts` now owns the grouped Search-root visual lane directly over the lower chrome-snap/reset, close-handoff, chrome-transition, foreground-visual, and shortcut-harness owners, `search-root-visual-runtime-contract.ts` now owns the shared root-visual type surface, and the final render surface now terminates under `search-root-render-runtime-contract.ts`, `use-search-root-runtime.ts`, `use-search-root-runtime.ts`, `use-search-root-runtime.ts`, and [`SearchRootRenderSurface.tsx`](/Users/brandonkimble/crave-search/apps/mobile/src/screens/Search/components/SearchRootRenderSurface.tsx) instead of another grouped render helper. That root-owned map/publication seam is flatter again now too: `runtime/shared/use-search-map-runtime.ts` now owns grouped map interaction plus stable handler composition, the deleted `runtime/shared/use-search-root-publication-runtime.ts`, deleted `runtime/shared/use-search-root-route-publication-runtime.ts`, deleted `runtime/shared/use-search-root-render-publication-runtime.ts`, deleted `runtime/shared/use-search-root-visual-publication-runtime.ts`, deleted `runtime/shared/use-search-root-publication-runtime-contract.ts`, and deleted `use-search-runtime-publication-runtime.ts` no longer sit between the root and the live publication/render lanes. The deleted `use-search-root-visual-publication-args-runtime-contract.ts` no longer owns another grouped visual-publication vocabulary because that surface now lives directly under `search-root-visual-runtime-contract.ts`, `use-search-results-sheet-visual-runtime.ts`, and `use-search-results-panel-visual-runtime-model.ts`. The deleted `use-search-root-chrome-input-publication-args-runtime-contract.ts` no longer owns another grouped chrome-input publication vocabulary because that surface now lives directly under `use-search-root-runtime.ts` and the surviving shared chrome contracts instead of another one-consumer helper family. `use-search-root-runtime-contract.ts` still owns the outward top-level root runtime surface, the deleted `search-root-map-render-publication-runtime-contract.ts` no longer owns another grouped map-render publication vocabulary because that surface now lives directly under `use-search-root-map-render-state-publication-args-runtime.ts` and `use-search-root-map-render-handlers-publication-args-runtime.ts`, and `search-root-render-runtime-contract.ts` owns the shared render vocabulary. The lower visual publication path now terminates directly through the root-owned visual lane in `use-search-root-runtime.ts`, `use-search-results-sheet-visual-runtime.ts`, `use-search-results-panel-visual-runtime-model.ts`, and inline suggestion/header/filter input assembly in `use-search-root-runtime.ts`. `use-search-root-runtime.ts` now owns the Search-origin restaurant host-config, panel, host-state, host-model, and publish/clear lane directly over `restaurantRoutePanelContract.ts`, `restaurantRouteHostContract.ts`, and `restaurantRouteRuntimeStore.ts`, and now composes those lower visual owners, Search-route publication, restaurant-route publication, final render assembly, and the now-direct root action/publication lane instead of routing through another grouped root-publication host.
...
- The Search-root pre-suggestion session/runtime lane is narrower too: the deleted `runtime/shared/use-search-root-session-runtime.ts`, deleted `runtime/shared/use-search-root-construction-runtime.ts`, deleted `runtime/shared/use-search-root-session-search-services-runtime.ts`, deleted `runtime/shared/use-search-root-session-overlay-map-runtime.ts`, deleted `runtime/shared/use-search-root-session-state-runtime.ts`, and deleted `runtime/shared/use-search-map-bootstrap-runtime.ts` no longer sit between the lower session owners and the real root. `runtime/shared/use-search-root-session-runtime-contract.ts` now owns the shared root-session vocabulary, and `runtime/shared/use-search-root-runtime.ts` now composes those lower session owners directly before the later suggestion/overlay/session/action lanes by owning the bus-backed runtime owner/state/flags/primitives/hydration lane itself, terminating the freeze/history/filter/request-status lane directly over `use-search-freeze-gate-runtime.ts`, `use-search-history-runtime.ts`, `use-search-filter-state-runtime.ts`, and `use-search-request-status-runtime.ts`, and terminating the overlay-command/bootstrap lane directly over `useSearchRouteOverlayCommandRuntime(...)` plus direct root-owned startup-camera/style-readiness/main-map-ready handling, instead of routing through another grouped session constructor shell.
- The Search-root overlay/session lane is narrower again too: `use-search-root-runtime.ts` now owns the full overlay-root/session/nav/visibility composition directly over inline overlay-store selection, direct `useSearchRouteSessionController(...)` composition, inline bottom-nav geometry caching, and inline root-owned docked-polls / nav-restore / final overlay-render policy, so `index.tsx` no longer physically hosts that lower overlay/session cluster inline and the scaffold contract now preserves that outward lane without another wrapper file.
- The Search-root map-movement/results-sheet lane is narrower too: the deleted `runtime/shared/use-search-results-sheet-runtime-lane.ts` no longer sits between the real root and the lower results-sheet owners, and `use-search-root-runtime.ts` now owns the Search-owned motion-pressure instance, map-movement policy, initial docked-polls-to-sheet state derivation, and the full lower results-sheet runtime composition directly over `use-results-sheet-shared-values-runtime.ts`, `use-results-sheet-visibility-state-runtime.ts`, `use-results-sheet-visibility-actions-runtime.ts`, and `use-results-sheet-visibility-sync-runtime.ts` while keeping runtime-model, animated-style, and grouped `resultsSheetRuntime` assembly inline at the real root boundary.
- That post-suggestion Search-root flow is narrower again too: `use-search-root-runtime.ts` now owns grouped session-shadow + request-runtime-owner/clear-owner/results-presentation-owner + autocomplete/recent/input composition directly over `use-search-session-shadow-transition-runtime.ts`, `useSearchRequestRuntimeOwner(...)`, `use-search-clear-owner.ts`, `useResultsPresentationOwner(...)`, `use-search-autocomplete-runtime.ts`, and `use-search-recent-activity-runtime.ts`, and `runtime/shared/use-search-root-action-runtime.ts` now owns grouped session-action + results-sheet-interaction + derived presentation-state composition over `use-search-session-action-runtime.ts` and `use-search-results-sheet-interaction-runtime.ts`. The remaining top-level Search-root constructor shell is narrower too: the deleted `runtime/shared/use-search-root-primitives-runtime.ts` and deleted `runtime/shared/use-search-root-suggestion-runtime.ts` no longer sit between the real root and the lower local state or suggestion visibility/layout owners because `use-search-root-runtime.ts` now owns the local map/search refs, setter state, store selection, root-local cache/selection/focus primitives, and direct suggestion visibility/layout composition itself while still deriving `isSuggestionScreenActive` inline. `use-search-root-runtime.ts` now terminates the scaffold lane directly over inline overlay-store selection, direct `useSearchRouteSessionController(...)` composition, inline bottom-nav geometry caching, `use-search-docked-polls-visibility-runtime.ts`, `use-search-nav-restore-runtime.ts`, `use-search-overlay-render-visibility-runtime.ts`, the direct root-owned results-sheet movement/runtime composition, and `use-search-runtime-instrumentation-runtime.ts`, and `search-root-scaffold-runtime-contract.ts` now owns the shared scaffold vocabulary instead of a one-consumer runtime wrapper or root-local arg-builder layer. The deleted mixed `runtime/shared/use-search-root-presentation-runtime.ts` host is replaced by `use-search-root-runtime.ts`, which now keeps that request/autocomplete/recent/input assembly inline while only sharing the narrower `search-root-request-lane-runtime-contract.ts` type surface with downstream consumers, the deleted mixed `runtime/shared/use-search-root-action-lane-runtime.ts` host is replaced by `runtime/shared/use-search-root-profile-action-runtime.ts`, which now owns the profile-specific selection/analytics/native-app execution lane, and `runtime/shared/use-search-root-session-action-runtime.ts`, which now stays on grouped session-action/results-sheet-interaction composition over the lower `use-search-root-session-action-args-runtime.ts`, `use-search-root-results-sheet-interaction-args-runtime.ts`, `use-search-root-presentation-state-args-runtime.ts`, and `use-search-root-action-runtime.ts` owners; the deleted `runtime/shared/use-search-root-display-runtime.ts`, deleted `runtime/shared/use-search-root-display-lane-runtime.ts`, and deleted `runtime/shared/use-search-map-runtime.ts` no longer sit between the real root and the lower map interaction/stable-handler owners, while the deleted `runtime/shared/use-search-root-flow-runtime.ts` and deleted `runtime/shared/use-search-root-flow-runtime-contract.ts` no longer sit between those lower suggestion/scaffold/request/action/map/publication phases and the top-level owner boundary. `runtime/shared/use-search-root-runtime-contract.ts` now owns the shared top-level root vocabulary, and `runtime/shared/use-search-root-runtime.ts` now composes session plus the lower suggestion/scaffold/request/action/map/publication owners directly, so `index.tsx` is now just the env/setup shell over that top-level root runtime plus `SearchRuntimeBusContext.Provider` and `SearchRootRenderSurface`.
...
- The last internal profile camera wrapper is gone too: `use-profile-camera-orchestration.ts` is deleted, and its capture/padding/commit logic now lives directly inside the live profile owner/runtime path instead of another single-use hook boundary inside the controller-era camera lane.
...
- The restaurant route contract is stronger too: Search-route runtime and global route producers no longer publish restaurant state as `panelOptions`, and now use a dedicated route-level `panel` contract that `RestaurantRouteLayerHost.tsx` passes straight into the native snapshot view path while host-only geometry stays separate. That contract is stronger again now too: producer-facing drafts carry native-ready `snapshotPayload` directly, and the route host no longer reshapes raw restaurant business data into the native snapshot payload at host read time.
...
- That Search clear ownership is narrower too: the inline `clearTypedQuery()` and `clearSearchState()` cluster no longer lives directly in `index.tsx`; `use-search-clear-owner.ts` now owns those Search clear semantics alongside `clearSearchAfterProfileDismiss()` under one grouped clear-owner surface, the extra `useSearchClearStateOwner(...)` export is deleted, and the grouped results presentation owner now consumes that unified clear-owner surface directly for close orchestration instead of making the root stitch clear + presentation + close together by hand. The Search-side request/action seam has moved again since the earlier audit pass: `use-search-root-runtime.ts` now delegates major request/action lanes through `use-search-root-request-lane-runtime.ts`, `use-search-root-action-lanes-runtime.ts`, and `use-search-root-scaffold-runtime.ts`, while `use-search-restaurant-route-publication-runtime.ts` owns the Search-origin restaurant host publication lane. `index.tsx` no longer directly calls the old lower request/clear/profile/submit/filter/foreground or results-sheet interaction owners, and the root is no longer relying on the older `use-search-session-action-runtime.ts` framing described in the earlier pass.
- That Search request/close ownership is narrower too: the root no longer keeps the
  `pendingCloseIntentIdRef` / `pendingCloseCleanupFrameRef` state machine, the inline
  `beginCloseSearch()` / `cancelCloseSearch()` / `finalizeCloseSearch()` controller body, a
  request-cancel bridge hook, or the `bindFinalizeCloseSearch(...)` effect that used to reconnect
  close completion back into the presentation shell. The extra `use-search-close-intent-owner.ts`
  wrapper is now deleted too, and the old `use-search-presentation-controller.ts` shell is deleted as
  well: the mixed `runtime/shared/use-results-presentation-shell-runtime.ts` host is now deleted, the
  visible Search-side presentation shell state is now split between
  `runtime/shared/use-results-presentation-shell-local-state.ts` and
  `runtime/shared/use-results-presentation-shell-model-runtime.ts`,
  `runtime/shared/use-results-presentation-owner-close-transition-lifecycle-runtime.ts`,
  `runtime/shared/use-results-presentation-owner-close-transition-actions-runtime.ts`,
  `runtime/shared/use-results-presentation-owner-close-search-cleanup-runtime.ts`,
  `runtime/shared/use-results-presentation-owner-presentation-actions-runtime.ts`,
  `runtime/shared/use-results-presentation-owner-sheet-execution-model-runtime.ts`, and
  `runtime/shared/use-results-presentation-owner-interaction-model-runtime.ts` now own the lower
  close-intent/cleanup, prepared enter/exit dispatch, grouped sheet-execution, and grouped interaction
  lanes directly. `runtime/shared/results-presentation-shell-runtime-contract.ts` now owns the grouped
  action vocabulary, and `runtime/shared/use-results-presentation-runtime-owner.ts` now stays on thin
  orchestration over those lower owners with the shared results runtime owner.
  `use-search-request-runtime-owner.ts` still owns the request lifecycle refs plus
  `cancelActiveSearchRequest()`, request failure/finalization policy, and the generic managed-attempt
  wrapper, and `index.tsx` now consumes one grouped results presentation boundary instead of wiring
  close orchestration back into it manually. That grouped boundary is narrower again now too: the
  deleted route/publication wrappers and deleted Search-only docked-polls publication shim no longer
  sit on the live path; the lower panel publication lane now lives directly under
  `runtime/shared/use-search-results-panel-data-runtime.tsx`,
  `runtime/shared/use-search-results-panel-chrome-runtime.tsx`,
  `runtime/shared/use-search-results-panel-list-selectors-runtime.tsx`,
  `runtime/shared/use-search-results-panel-list-layout-runtime.tsx`,
  `runtime/shared/use-search-results-panel-list-publication-runtime.tsx`,
  `runtime/shared/use-search-results-panel-read-model-runtime.tsx`,
  `runtime/shared/use-search-results-panel-render-policy-runtime.tsx`,
  `runtime/shared/use-search-results-panel-covered-render-runtime.tsx`,
  `runtime/shared/use-search-results-panel-surface-state-runtime.tsx`,
  `runtime/shared/use-search-results-panel-interaction-frost-runtime.tsx`,
  `runtime/shared/use-search-results-panel-surface-background-runtime.tsx`,
  `runtime/shared/use-search-results-panel-surface-overlay-runtime.tsx`,
  `runtime/shared/use-search-results-panel-spec-runtime.tsx`, and
  `runtime/shared/use-search-results-panel-route-visibility-runtime.tsx`, while the overlay-owned route
  host lane is now split more honestly between `useSearchRouteOverlayPublishedState.ts`,
  `useSearchRouteOverlaySheetKeys.ts`, the deleted `useSearchRouteOverlayPanelSpecFamilies.ts` wrapper,
  the deleted `useSearchRouteOverlayPanelSpecsRuntime.ts` wrapper,
  `useSearchRouteOverlayActiveSheetSpec.ts`, `useSearchRouteOverlaySheetVisibilityState.ts`, the deleted
  `useSearchRouteHostModelInput.ts`, `useSearchRouteOverlayRenderPolicy.ts`,
  `useSearchRouteOverlayRuntimePublication.ts`, and the thinner `useSearchRouteOverlayRuntime.ts`
  composition hook over the root-composed panel spec and the remaining overlay-host policy inputs. The
  Search root now crosses one `shellModel` plus grouped presentation/close-transition actions instead
  of another flat bag of results-shell props.
...
- `profile-owner-runtime.ts`
- `RestaurantRouteLayerHost.tsx`
- `index.tsx`

Current shape:

- pin press enters the profile owner/runtime path and may also publish the dedicated restaurant route host model when the route switches to restaurant
...
Live shape note: the deleted `use-search-root-publication-runtime-contract.ts` no longer owns a grouped root-publication vocabulary. The remaining root/publication types now terminate under the real lower owners: `use-search-root-runtime-contract.ts` owns the outward top-level root runtime surface, `search-root-environment-contract.ts` now owns the explicit external `SearchRootEnvironment` together with the narrower environment subcontracts for the surviving root-side owners and lower foreground arg-builders, `search-root-core-runtime-contract.ts` now stays on the shared root suggestion type surface instead of another grouped `SearchRootCoreRuntime` bag, the deleted `use-search-root-visual-publication-args-runtime-contract.ts` no longer owns another grouped visual-publication vocabulary because that type surface now terminates directly under `use-search-root-runtime.ts` and `search-root-visual-runtime-contract.ts`, the deleted `use-search-root-chrome-input-publication-args-runtime-contract.ts` no longer owns another grouped chrome-input publication vocabulary because that type surface now terminates directly under `use-search-root-runtime.ts` and the surviving shared chrome contracts instead of a separate suggestion/header helper family, the deleted `search-root-map-render-publication-runtime-contract.ts` no longer owns another grouped map-render publication vocabulary because that type surface now terminates directly under `search-root-render-runtime-contract.ts` and `use-search-root-runtime.ts`, and `search-root-render-runtime-contract.ts` now owns the shared render vocabulary together with the outward grouped `SearchRootMapRenderSurfaceModel`, `SearchRootOverlayRenderSurfaceModel`, and `SearchRootModalSheetRenderSurfaceModel` surface plus direct component-prop surfaces instead of hook-return contracts. The deleted `use-search-runtime-publication-runtime.ts` wrapper no longer sits on that lane because `use-search-root-runtime.ts` now terminates the runtime publication lane directly at the real root action owner. The deleted `use-search-root-presentation-surface-visual-runtime.ts`, deleted `use-search-root-presentation-owner-runtime.ts`, deleted `use-search-root-visual-runtime-args-runtime.ts`, and deleted `use-search-root-render-lane-runtime.ts` no longer own grouped visual/input publication preparation or route/render handoff, while `use-search-root-runtime.ts` now owns that direct results-sheet visual state, results-panel visual model, Search-route publication, and restaurant-route publication lane over inline root-owned results-sheet visual state/diagnostics, `use-search-results-panel-visual-runtime-model.ts`, `use-search-route-panel-publication-runtime.ts`, direct restaurant-route panel/host-state assembly over `overlays/restaurantRoutePanelContract.ts` and `overlays/restaurantRouteHostContract.ts`, and direct restaurant host-model publication into `overlays/restaurantRouteRuntimeStore.ts`. `use-search-root-runtime.ts` now owns the direct map-prop lane over inline `map-interaction-controller.ts` plus the inline stable map-handler ref layer, publishing into `mapRenderSurfaceModel` without another grouped `coreRuntime` handoff. `use-search-root-runtime.ts` now owns the grouped root visual lane directly over `use-search-overlay-chrome-snaps-runtime.ts`, `use-search-overlay-sheet-reset-runtime.ts`, `use-search-close-visual-handoff-runtime.ts`, `use-search-chrome-transition-runtime.ts`, `use-search-foreground-visual-runtime.ts`, and direct shortcut-harness ref publication at the real root boundary, while `search-root-visual-runtime-contract.ts` owns the shared root-visual type surface. `use-search-root-runtime.ts` now directly composes inline suggestion/header/filter input assembly, direct chrome render-model composition, and direct bottom-nav prop assembly, publishing the root-facing overlay boundary into `overlayRenderSurfaceModel` without another grouped `coreRuntime` boundary. `use-search-root-runtime.ts` now publishes the sheet boundary into `modalSheetRenderSurfaceModel` through direct rank-and-score and price-sheet prop assembly instead of `use-search-rank-and-score-sheets-props.ts` or `use-search-price-sheet-props.ts`, and `use-search-root-runtime.ts` now directly composes its direct map lane, the direct root visual lane, the direct route/restaurant publication lane, the direct foreground chrome lane, and the direct modal-sheet prop lane instead of routing render/publication work through another intermediate lane owner. `SearchRootRenderSurface.tsx` now consumes those three grouped render-surface models directly instead of a final flat prop explosion at the screen boundary.

Live shape note: the shared results owner/runtime seam is flatter again too. The deleted `use-results-presentation-runtime-core-owner.ts`, deleted `use-results-presentation-owner-close-exit-runtime.ts`, deleted `use-results-presentation-owner-enter-request-runtime.ts`, deleted `use-results-presentation-owner-toggle-runtime.ts`, deleted `use-results-presentation-owner-close-search-cleanup-runtime.ts`, deleted `use-results-presentation-owner-sheet-execution-model-runtime.ts`, and deleted `use-results-presentation-owner-interaction-model-runtime.ts` now also remain gone. `use-results-presentation-runtime-machine-owner.ts` now owns the runtime-machine publication, prepared snapshot staging/commit policy, and marker enter/exit handoff coordination directly under the real owner, while `use-results-presentation-toggle-runtime.ts` now owns toggle lifecycle plus frost-ready completion over that lower machine owner. `use-results-presentation-owner-close-transition-lifecycle-runtime.ts` now owns pending/active close intent refs plus restore/finalization lifecycle policy. `use-results-presentation-owner-close-transition-actions-runtime.ts` now owns close-transition event application plus grouped `closeTransitionActions` over that lower lifecycle lane. `use-results-presentation-runtime-owner.ts` now keeps outward `beginCloseSearch()` / `handleCloseResults()` / `cancelCloseSearch()` inline at the real owner boundary, assembles grouped `resultsSheetExecutionModel`, prepared exit execution, prepared snapshot shell application plus prepared enter/editing/close request intent dispatch, deferred close-search cleanup scheduling, and tab-toggle commit plus grouped `interactionModel` inline, and stays on shell-local state, shell model, and thin outward owner composition over those lower runtime and owner-action lanes instead of physically hosting that whole mixed owner lane inline.
...
Live shape note: the shared results owner-side action lane is flatter again too. The deleted
`use-results-presentation-shell-close-runtime.ts`, deleted
`use-results-presentation-shell-request-runtime.ts`, deleted
`use-results-presentation-shell-actions-runtime.ts`, deleted
`use-results-presentation-close-search-runtime.ts`, deleted
`use-results-presentation-close-transition-state-runtime.ts`, deleted
`use-results-presentation-close-intent-runtime.ts`, deleted
`use-results-presentation-close-transition-lifecycle-runtime.ts`, deleted
`use-results-presentation-close-transition-actions-runtime.ts`, deleted
`results-presentation-shell-actions-runtime-contract.ts`, deleted
`use-results-presentation-owner-close-runtime.ts`, deleted
`use-results-presentation-owner-request-runtime.ts`, deleted
`use-results-presentation-owner-close-transition-runtime.ts`, deleted
`use-results-presentation-owner-close-search-runtime.ts`, deleted
`use-results-presentation-owner-close-transition-state-runtime.ts`, deleted
`use-results-presentation-owner-sheet-execution-runtime.ts`, deleted
`use-results-presentation-owner-close-exit-runtime.ts`, deleted
`use-results-presentation-owner-enter-request-runtime.ts`, and deleted
`use-results-presentation-owner-toggle-runtime.ts` remain gone.
`use-results-presentation-owner-close-transition-lifecycle-runtime.ts` now owns direct
close-transition refs plus restore/finalization policy over its direct dependencies,
`use-results-presentation-owner-close-transition-actions-runtime.ts` now owns grouped
`closeTransitionActions`, `use-results-presentation-owner-presentation-actions-runtime.ts` now owns
prepared snapshot shell application plus `requestSearchPresentationIntent(...)` and the outward
presentation actions over its direct dependencies, and `use-results-presentation-runtime-owner.ts`
now stays on thin orchestration over those real owner-side lanes plus shell-local state, shell
model, and direct runtime-machine/toggle composition instead of physically keeping grouped
sheet-execution ownership, prepared enter/exit planning, or grouped interaction ownership inline at
the top-level owner.
...
> **CORRECTION 2026-08-05 (F1496).** `useActiveSearchRestaurantRouteRestaurantId()` no longer exists — it had zero production consumers and was deleted 2026-08-05. Highlighted-restaurant identity is still not duplicated in local React state (that half of the claim holds); the specific hook named here is simply gone. Recorded rather than rewritten, so the supersession is visible.
