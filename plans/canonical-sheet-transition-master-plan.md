...
> **[SUPERSEDED — banner added 2026-08-03 (truth audit)]** The header below still reads
> "Status: IN EXECUTION"; that is false as of today. This doc (2026-06-27) was superseded two
> days later by `transition-engine-final-master-plan.md`, whose own header names this file in its
> supersedes list ("Supersedes sheet-transition-engine-design.md, page-transition-and-results-
> engine.md, transition-pillars-build-plan.md, transition-engine-increment-1.md,
> canonical-sheet-transition-master-plan.md"). That plan was in turn superseded by
> `page-switch-master-plan.md` (2026-07-01, "Canonical source of truth… SUPERSEDES the
> crossfade/fold-up lineage"), then by `page-composition-from-scratch-design.md` (THE PAGE v2,
> 2026-07-16 → the TrackSheet cutover), `page-world-derivation.md` (2026-07-29) and
> `residents-cutover-plan.md` (2026-08-01). Phases 6 and 7 in §6 were never reached under this
> plan's terms. Read the ✅ execution log below as a true record of what landed in that window,
> not as the live roadmap.
>
> **Correction 2026-08-03 (truth audit):** the acceptance criteria at lines ~276, ~405 and ~460
> that read the `[lodev]` harness (`renderP`, `roleP`, lodev `frame`/`step` events) are
> unrunnable — the `[lodev]` telemetry emitter DOES NOT EXIST in this repo. A repo-wide grep for
> `lodev` over `apps/` returns zero hits in any `.swift`/`.ts`/`.tsx` source; the only live map
> telemetry is the inert `[LODDBG]` NSLog probes behind
> `SearchMapRenderController.swift`'s `lodDebugLoggingEnabled = false`. Same correction already
> recorded in `plans/lod-ideal-residency-refactor.md` §CORRECTION 2026-08-03.

**Status (as written 2026-06-27, now historical):** IN EXECUTION. Supersedes `canonical-transition-finish-plan.md`,
...
The red-team's central finding — _only the search-surface redraw path emits readiness gates today_
...
cause of all four Bug-1 failures: restaurant-from-comment inherited a _different surface's_ defaults.
...
6. **Every reveal is a PUSH; the whole chain is push-based.** `[polls, pollDetail, restaurant]` must
   be a real stack so dismiss can pop to the exact origin. The push rule applies to the _poll-open
   that precedes the comment tap_, not only the restaurant reveal. No reveal uses `setRoot`
...
**The link the red-team proved is missing:** `markSceneContentGate` keys on the _redraw_
`transactionId` (`"search-surface-results-transaction:N"`); the motion plane keys on the _settleToken_.
...
3. **Check-on-arm**: search gates can close _before_ the switch goes in-flight (data loads during
...
**The red-team's critical constraint (drives the whole phase order):** _only `search-surface-runtime.ts`
emits gates today_ — exactly three producers (`:606/618/624` → `cards`, `nativeMarkerFrame`, `sheet`),
...
content contract.** The only producer work needed is on the _committed restaurant search_ (which reuses
the existing search-family `cards/nativeMarkerFrame/sheet` producers) and the _dismiss restore gates_
...
`restaurant` is REMOVED from the seeded set.** A restaurant reveal is a _committed single-restaurant
search presented as a profile_ — `seeded:false`, `chrome:'results'`, `mapSource:'committed'`. See §4 for
...
`revealRoute(descriptor)` runs three deterministic phases — seeded-vs-loading is a _branch inside
ADMIT_, not a different lifecycle:
...
**return target on dismiss**. Rows 2/3/4/6 are _the same operation with different payloads_.
...
- **Snap.** Two legal values only: `preserveLiveY` _only_ when origin and target share the same live
...
- **Map/pin.** All "committed" — including row 3. The restaurant is the _committed result_
...
**Fix:** the restaurant _is_ the committed result (`length === 1`) → it renders via the normal committed
...
**Critical red-team caveat (must-fix 4):** returning to a poll comment _is_ returning to a poll, so the
...
_insufficient_ — the crashing transition is `{restaurant→pollDetail-comment}`, a different transition
...
- **(B) forward-open preserve/relabel/hold cluster** (`routeSwitchPreservedOutgoingSheetSceneKey`,
  ...
  _names_ the held leg (which must be `pollDetail` for restaurant-from-comment). Pin the held-leg
...
weld. For the polls-collapsed case the contract resolves to the _exact same gate set_, so the emitted
bytes are unchanged. Other dismiss targets (restaurant→pollDetail-comment) declare _their_ restore gates.
**Constraints:**
...
exist: every reveal rides the search surface, and the restaurant profile rides it as a _presentation of
a committed result_. Search-from-anywhere is the search bar's own gate; unified dismiss is a back-stack
...
- **DoD (harness):** search-to-results logs `[READYGATE] content-ready` _before_ settle; `renderP` /
...
**Requirement (owner).** On _every_ restaurant-profile reveal — the entity/comment reveal AND the regular
