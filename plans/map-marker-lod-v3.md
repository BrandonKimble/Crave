...
- **LOD**: map renders a mix of *full pins* (top N) + *dots* (the rest) with **snap** transitions (no animation yet).
...
3) `distanceMiles ASC` (only meaningful if we define “center” distance; also changes with viewport)
