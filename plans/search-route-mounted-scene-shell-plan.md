...
>
> **Correction 2026-08-03 (truth audit), file-specific:** the mounted-scene migration
> named here (Slices A–F, `PollsScene`/`BookmarksScene`/`ProfileScene`) did not land in
> this shape — no `*Scene` component exists under `screens/Search/runtime/`, and
> `apps/mobile/src/overlays/SearchRouteMountedSceneOwners.tsx` is absent. The scene model
> that actually shipped is the route scene-stack runtime
> (`apps/mobile/src/navigation/runtime/app-route-scene-stack-runtime.ts`,
> `app-route-scene-switch-controller.ts`, `transition-engine/`), per the July lifecycle
> line. "Status: active" (2026-04-14) is stale.
