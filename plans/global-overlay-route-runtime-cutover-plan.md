...
- restaurant still shares the common sheet motion/chrome primitive, but that remaining shared layer is now explicitly isolated under `RestaurantRouteLayerHost.tsx` + `OverlaySheetShell.tsx` instead of older dedicated `RestaurantOverlayHost.tsx` / `RestaurantSheetHost.tsx` wrappers
...
- the restaurant producer contract is more uniform too: `restaurantRouteRuntimeStore.ts` no longer stores global restaurant state as a split `panel + sessionToken` path while Search publishes through a different wrapper-owned route publisher. Global and Search restaurant publications now both terminate on the same store-owned publication contract, `use-search-restaurant-route-publication-runtime.ts` now publishes the Search-side model directly, and the old `useSearchRestaurantRoutePublisher.ts` wrapper is deleted
- the restaurant route payload boundary is stronger too: the Search-side owner and the reusable non-Search producer now both publish native-ready `snapshotPayload` panel drafts through `restaurantRoutePanelContract.ts` instead of leaking raw restaurant business data through the overlay route contract, so `RestaurantRouteLayerHost.tsx` reads a dedicated route-host model instead of rebuilding restaurant/profile payload policy inline at host read time
- the restaurant route host boundary is stronger too: `RestaurantRouteLayerHost.tsx` now resolves one dedicated `RestaurantRouteHostModel` through `restaurantRouteHostContract.ts` plus `useResolvedRestaurantRouteHostModel.ts` before mounting the host, rather than depending on older wrapper-specific host wiring
- the restaurant host/runtime split is narrower too: the route-specific payload/model work now terminates in `RestaurantRouteLayerHost.tsx`, while the remaining shared sheet motion/chrome execution stays under `OverlaySheetShell.tsx` instead of a deleted `RestaurantOverlayHost.tsx` / `RestaurantSheetHost.tsx` stack
...
3. then reassess only the remaining bridge-edge typing and any measured UX/perf issues instead of reopening the deleted `RestaurantOverlayHost.tsx` / `RestaurantSheetHost.tsx` stack.
...
- Search no longer mounts `OverlaySheetShell` or any restaurant route host directly
...
- `RestaurantRouteLayerHost.tsx` is now mounted directly from the app-level host when the active overlay route is `restaurant`; it is no longer rendered from the Search sheet tree branch
...
- the restaurant route now uses a dedicated host-model builder in `/Users/brandonkimble/crave-search/apps/mobile/src/overlays/RestaurantRouteLayerHost.tsx` and `/Users/brandonkimble/crave-search/apps/mobile/src/overlays/useResolvedRestaurantRouteHostModel.ts`, not a generic `OverlayContentSpec` abstraction
- the remaining restaurant shared-sheet seam is narrower too: `RestaurantRouteLayerHost.tsx` no longer directly owns the native sheet/scroll/motion host runtime, and that shared host layer now lives under `OverlaySheetShell.tsx` while the route’s visible content/payload model stays dedicated end-to-end. The lower native host assembly, command/snap callback bridge, and shared scroll/momentum runtime still live under the shared sheet host stack, but the restaurant-visible lane now stays dedicated through the route host/model path instead of older restaurant-specific wrapper hosts
