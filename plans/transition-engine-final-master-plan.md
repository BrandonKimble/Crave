> **[SUPERSEDED — banner added 2026-08-03 (truth audit)]** Despite the "FINAL" in the title,
> this plan was extended-and-superseded two days later by `page-switch-master-plan.md`
> ("Canonical source of truth… It EXTENDS `transition-engine-final-master-plan.md` with the
> nav-page specialization the final-master-plan under-specified"), and the whole lineage was then
> replaced by `page-composition-from-scratch-design.md` (THE PAGE v2, 2026-07-16 → the ONE TRACK
> / TrackSheet cutover), `page-world-derivation.md` (2026-07-29) and `residents-cutover-plan.md`
> (2026-08-01). Phases 4–6 below were never executed as written.
>
> **Correction 2026-08-03 (truth audit):** §6's acceptance gate G2 ("Read the LOD JSONL harness
> — `[lodev]` events: `step`, `frame`, `cwork` — it is the screen, quantified") is not runnable:
> the `[lodev]` harness DOES NOT EXIST in this repo. A repo-wide grep for `lodev` over `apps/`
> returns zero source hits; the only map telemetry is the inert `[LODDBG]` NSLog probes behind
> `SearchMapRenderController.swift`'s `lodDebugLoggingEnabled = false`. Same correction recorded
> in `plans/lod-ideal-residency-refactor.md`. §5's Bug-#1a fix (hoist ONE opaque frost backing;
> holes punch through to it) did land and hardened further into THE TRUE-CUTOUT LAW — see
> `page-composition-from-scratch-design.md` §THE TRUE-CUTOUT LAW (2026-07-23, shipped).
...
**North Star.** Every transition in this app — all permutations, forward _and_ reverse —
...
- **I1 — Slide, never snap.** The sheet maintains its current detent OR travels to a target
  detent via one continuous, velocity-matched glide. The detent is the _target_; the motion
  to it is always a glide. `from == to` ⇒ the sheet _stays put_ (no motion at all). No
  `snapTo`-teleport, no fixed-duration jump, never `{kind:'hide'}` to nothing.
- **I2 — Never see-through, ever.** The sheet _surface_ alpha is a constant `1.0`. Nothing
  the engine controls can lower it. The map is visible only where the sheet's _geometry_
  doesn't cover it — never _through_ it. (Gate G1: min surface alpha == 1.0 across the whole
  ramp.)
- **I3 — Press-up is the sole trigger.** On the gesture's terminal event, all motion lanes
  (sheet, map, chrome) start _in the same UI-thread frame_. Nothing waits on async
  data/readiness/network to _begin_. Readiness governs only _when incoming content becomes
  visible inside the already-moving opaque sheet_ — never when the sheet/map move.
- **I4 — Motion is independent of content.** Sliding the sheet is a `translateY` on an opaque
  surface; swapping content is paint _inside_ that surface. Orthogonal channels that may run
...
- **I7 — Trivially extendable.** Adding a flow (e.g. profile-page list → search) is **one new
  descriptor row**, zero engine edits. The descriptor table is the _sole_ per-scene authority.
- **I8 — Interruptible & reversible from the live state.** Any in-flight transition can be
  grabbed at any frame; a new gesture seizes the _live_ presented value + velocity of every
...
1. **Opaque container + content-only cross-dissolve.** Material Design 3 / `MaterialContainerTransform`:
   _"the container background color is always drawn as fully opaque, beneath all other content
   in the container"_ — only the contents transition. gorhom RN bottom-sheet decouples the
   sheet _surface_ from content (surface sized to cover the sheet independent of content
...
2. **The 0.75-midpoint proof (why naive crossfades leak).** Jake Archibald: two source-over
   ...
   always interpose the opaque backing. Over an opaque backing the 25% reveals the _backing_,
   not the map — safe. For a mathematically dip-free dissolve against a _textured_ frost, use
...
4. **Padding-as-truth non-modal map sheet.** The sheet does NOT cover the map's center; its
   ...
   transition). Drive padding from the _same_ shared value as sheet-Y so they're frame-locked.
...
welds content teardown to a _collapse boundary_ the sheet must physically reach first
...
(`BottomSheetSceneStackHost.tsx:929`) as a sibling _below_ the content layers; set
...
stay per-scene _above_ the shared plate — only the flat opaque fill is hoisted.
...
opacity. This makes "map shows through" _unrepresentable_.
...
**One resolved target detent (red-team MUST-FIX C).** `progress` is _normalized travel toward
the resolved target detent_. The resolved target = `to` for programmatic taps (every matrix
...
- **`hard`** (seedable scenes: pollDetail, restaurant profile via direct-seed). On press-up,
  replace Layer-1 content with the incoming scene in one frame — _but the visible commit is
  gated on a single paint-ack_ (the incoming scene is pre-mounted offscreen and must emit its
...
- **`held-dissolve`** (NON-seedable scenes: all results / favorites-results — a results list
  cannot seed believable rows without the query response). Hold the _outgoing_ content fully
  ...
  done _correctly_ — content-only, over an opaque backing, paint-gated — the one good idea
...
shifts only _where_ `center` renders. Drive `paddingBottom` off the same `progress`/`sheetY`.
...
type OriginRef = {
  // EVERYTHING needed to reverse-mount, exactly
  sceneKey: OverlayKey;
  snap: Detent;
  scrollOffset?: number;
...
const inverse = (d: TransitionDescriptor): TransitionDescriptor => ({
  ...
  content: {
    ...d.content,
    out: d.content.in,
    in: d.content.out ?? d.origin /* never null on a real dismiss */,
  },
...
opacities interpolate over `swap.threshold`, _inside_ the surface, above the locked-opaque
...
**Deadlock seam (red-team MUST-FIX E), direction-agnostic.** A transition that emits _no_
...
`sheet from==to` ⇒ the sheet _stays_ (content/map/chrome still play). Dismiss = `inverse(row)`
...
\* **Row 6 owner decision (do not guess).** Result-list _dish_ tap today opens the dish's
_restaurant profile_ (`dish-result-card.tsx:143`), contradicting the brief's "dish entity →
...
handle); content lanes dissolve _over_ it (`held-dissolve`) or hard-swap. Surface alpha is a
constant 1.0 ⇒ map is unrepresentable inside the sheet. **Gate G1.** Lands in Phase 0 — fixes
see-through on the _existing_ engine immediately, before the rest of the rewrite.
...
physically reaching the _collapsed boundary_ (`finalizeCloseTransition` ←
...
`hide`; content swaps back to the _exact_ origin (comment/card/list/feed via `childAnchor`)
...
result-list direct seed), and the engine asserts the entity feature is in the source _before_
...
- **DoD:** record rows 3 / 7 / 1 reveals; scrub frame-by-frame; **G1 passes** — map never
  appears inside the sheet at any `progress`, even on the _legacy_ ramp. Independently shippable.
...
- **DoD:** all 13 reveals on-device; **issue #2** pin present _before_ `fitPin` (`roleP`/`renderP`
...
- **Revert from current session:** the bolt-on child-origin Lane-B as a _special case_ is
...
- **GATE (red-team MUST-FIX D — prove, don't assert):** the repo is mid-migration to a
  ...
  animation (it already tolerates pan/zoom camera moves, so padding-as-camera-move is _likely_
...
- **Revert / retire:** the `preserveOutgoingUntilSettle` content-plane crossfade and the
  ...
  `{kind:'hide'}` lane; the child-origin dismiss as a _special-case fork_ (generalized instead);
...
1. **Row 6 — result-list DISH tap.** Today it opens the dish's _restaurant profile_
...
5. **Favorites dismiss fidelity.** Should favorites dismiss restore the _exact scrolled list
   position_ (full `OriginRef` with `scrollOffset`), or is returning to the bookmarks root @snap
   acceptable? (The new model supports exact restore; confirm it's wanted.)
6. **Autocomplete-poll dismiss target (row 10).** Dismiss back to the _search_ surface, or
   straight to the _polls feed_ (poll-dismiss semantics)? Affects the `origin` for that row.
