...
### Decay — ✅ BUILT (the projection-emits-records architecture)

- **Exponential, ONE half-life** (`endorsementHalfLifeDays`, default 365 — ~12–18 mo
  community-memory horizon), weight `power(0.5, age_days / halflife)`, `age_days` from each
  mention's `mentioned_at` (= the Reddit post date; verified). Kills the old mention-365/upvote-240
  asymmetry. The Reddit phase-out engine.
- **Architecture (built):** `projection-rebuild` emits one **decay-ready record per counted
  contribution** into `core_restaurant_item_mentions` (connection, `mentioned_at`, `source_upvotes`,
  `kind` = direct|support) — preserving the fan-out + attribute gating it already computes — and the
  scorer **decay-sums those records** for dishes at rebuild; praise decay-sums `core_restaurant_events`
  directly (no fan-out there). No materialized decayed score (rows are raw; decay is applied fresh
  each rebuild → always current). This is NOT "scorer reads raw events" (Step 2 — reverted, couldn't
  carry the fan-out).
- **Correctness proof (no age-spread today):** an **equality gate** in the fixture validator
  confirms the records reproduce the projection's counts exactly (0 mismatches across all 1,175
  connections — 1,358 direct + 668 support), and a **synthetic-age** check confirms the weight
  halves every half-life. The fan-out is preserved (Caffè Panna's dishes keep their +12 support and
  score 99.8+).
- **Inert today, by design:** the corpus is a ~2-week slice (chronological collection), so every
  weight ≈ 0.95 → decay does nothing yet and the half-life is untunable. At launch the archive
