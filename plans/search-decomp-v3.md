...
- 2026-02-17: Track B candidate `trackb-hard-phase-lanes-pass1` kept (`stallMaxMean 226.533 -> 212.4`, catastrophic remained zero, parity held); auto-advanced to Track C because first-stall p95 remained above `60ms`.
- 2026-02-17: Track C candidate `trackc-runtime-bus-map-lane-pass1` reverted (`stallP95 191.6 -> 235.075`, `stallMaxMean 212.4 -> 253.9`); no dependency unlock justified keeping regression.
- 2026-02-17: Track C candidate `trackc-lane-signal-ref-pass2` kept after hard ownership cut removing root state lane writes (`stallP95 191.6 -> 111.27`, catastrophic stayed zero, parity held); continue Track C for first/worst p95 reduction.
- 2026-02-17: Track C candidate `trackc-map-signal-ref-only-pass3` reverted (`stallP95 111.27 -> 174.06`, `stallMaxMean 227.667 -> 236.3`); ref-only map lane props did not hold gains.
- 2026-02-17: Track C candidate `trackc-chrome-freeze-hydration-pass4` reverted (`stallP95 111.27 -> 200.05`, `stallMaxMean` flat), indicating freeze-window broadening moved overlap rather than reducing it.
- 2026-02-17: Track C candidate `trackc-overlay-runtime-freeze-pass5` reverted after two invalid/unstable run sets (`316a` completed `2/3`, `25fd` completed `1/3`) and severe partial regression (`stallP95 219.75`, `stallMaxMean 361.65`, catastrophic recurrence); treat as fake split/timing shift and continue with hard ownership cuts only.
- 2026-02-17: Track C candidate `trackc-marker-domain-ref-pass6` reverted (`stallP95 111.27 -> 270.72`, `stallMaxMean 227.667 -> 329.9`, catastrophic `0 -> 2`); moving marker contract off root state without preserving staged admission worsened overlap and was not a valid ownership win.
- 2026-02-17: Track C candidate `trackc-results-sheet-bus-domain-pass7` reverted (`stallP95 111.27 -> 304.02`, `stallMaxMean 227.667 -> 329.8`, catastrophic `0 -> 1`) and every run timed out at settle (`~45s`), indicating deferred bus ownership moved work into longer `pre_response_activation` windows rather than reducing overlap.
- 2026-02-17: Track C candidate `trackc-hydration-admission-ref-pass8` reverted (`stallP95 111.27 -> 223.72`, `stallMaxMean 227.667 -> 251.1`) with parity intact but no dependency unlock; ref/snapshot admission shifted timing while dominant `SearchScreen` + `SearchResultsSheetTree` overlap remained and all runs still settled by timeout (`~45s`).
- 2026-02-17: Control run `trackc-control-post-pass8-revert` established immediate baseline (`stallP95 105.69`, `stallMaxMean 260.367`, catastrophic `1`) with matched signature and parity intact.
- 2026-02-17: Track C candidate `trackc-hydration-step-policy-pass9` reverted (`stallP95 105.69 -> 152.215`, `stallMaxMean 260.367 -> 216.067`, catastrophic `1 -> 0`); mixed outcome failed first-stall gate and did not qualify for keep without a dependency unlock.
- 2026-02-17: Track C candidate `trackc-hydration-step-policy-pass10` reverted (`stallP95 105.69 -> 125.955`, `stallMaxMean 260.367 -> 232.033`, catastrophic `1 -> 0`); first-stall direction regressed and slice was timing-policy only (no structural dependency unlock).
- 2026-02-17: Track C candidate `trackc-hydration-admission-bus-domain-pass11` reverted (`stallP95 105.69 -> 164.535`, `stallMaxMean 260.367 -> 292.033`, catastrophic `1 -> 2`); bus-domain admission shift increased catastrophic overlap and did not justify keep.
- 2026-02-17: Ownership consolidation batch started (no intermediate gates per user instruction): removed root callback mediation for results finalize lane, moved map staged-publish defer control to signal-ref ownership (`deferMapPressureSignalRef`/`deferMapPinsSignalRef`/`deferMapFinalizeSignalRef`), migrated hydration-admission ownership to runtime bus selectors, and moved results-domain read inputs (`results`/`activeTab`/loading/submitted query) from root panel-spec fanout to runtime bus consumption; validation deferred until batch-complete.
- 2026-02-17: Ownership consolidation batch continued (still ungated): completed a single consolidated runtime-bus publish path in `SearchScreen` (base runtime contract + hydration admission in one effect) and moved additional results-domain ownership (`canLoadMore`, `activeOverlay`) from root props into bus selectors; no intermediate perf harness run per user instruction.
- 2026-02-17: Ownership consolidation batch continued (still ungated): moved on-demand notice derivation from `SearchScreen` into `useSearchResultsPanelSpec` (results-domain ownership), removing another results-derived prop from root fanout.
- 2026-02-17: Ownership consolidation batch continued (still ungated): moved results filter/header chip-state and gating booleans into runtime-bus ownership and removed them from root `searchResultsPanelSpecArgs` fanout; keeps results-domain reads local and narrows root/results coupling.
- 2026-02-17: Ownership consolidation batch continued (still ungated): removed root-mediated map/results lane refs entirely; `SearchMap` now subscribes to runtime-bus lane state directly and results finalize-lane state is published from results domain (`useSearchResultsPanelSpec`) into the bus.
- 2026-02-17: Perf check `trackc-ownership-consolidation-batchA-check1b` (3 runs) completed after ungated ownership batch; all runs hit settle timeout at `~45s`, first/worst stalls remained in `pre_response_activation` with regression (`stallP95 290.9`, `stallMaxMean 286.267`, catastrophic windows present), so batch is not promotable yet.
