...
| `FOOD_CATEGORY_EDGE_BUILDER_ENABLED` | Nightly food-category edge rebuild | Same. **HARD ORDER (audit 2026-08-31): flip ONLY after `DISH_KNOWLEDGE_SYNTHESIS_ENABLED` has finished its backfill and `knowledge_categories` is populated** — the builder full-replaces from that facet and its zero-input scream is silenced, so arming it against an empty facet silently WIPES every standing category edge (R6 class, live: 4,839 edges vs 0 populated facets on 2026-08-31). Gate on the backfill being COMPLETE, not merely started — a partial facet (proven live 2026-08-31: 11 of 3,910 dishes after a 20-dish test pass) passes any non-empty check and the full replace would legally shrink 4,839 edges to ~14. The gate: `SELECT count(*) FROM core_entities WHERE type='item' AND status='active' AND (knowledge_prompt_version IS DISTINCT FROM 4)` must be 0 (no dish still owes synthesis) | Launch, AFTER synthesis backfill completes |
