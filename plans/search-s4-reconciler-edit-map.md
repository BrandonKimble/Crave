...
- COLLAPSE `lastEnterRequestKey`/`lastEnterStartedRequestKey`/`lastEnterSettledRequestKey`/`lastDismissRequestKey` into one `(currentWorldId, currentPhase)` register; retarget algebra: any new worldId at any phase re-ramps from current opacity; a reveal assertion clears nothing because there is no dismiss key (task #16 dies here).
  - **Scoping facts from S4d-3b/3c-s1 (2026-07-08):** every timer/bypass hold regime is ALREADY dead (nothing here needs atomicity anymore), and the five payload request keys are already deleted. What remains is shape: started/settled become phase LEVELS of the one register (enter guards read them as `phase >= enterStarted` predicates); the dismiss transaction key (`search-surface-results-transaction:N`) survives ONLY as an ack correlation LABEL (charter-legal), never lifecycle — its lone JS consumer is the render owner. The real breadth is the EVENT-KEY ECONOMY: ~30 native emit sites carry `requestKey` with JS consumers (readiness logs, marker engine, seam settle handlers) — the collapse must rename event identity to worldId across emitter + consumers in one pass, so budget a full focused session with the complete regression lane (submit/toggle/tab/dismiss/resubmit/cache-hit resubmit).
