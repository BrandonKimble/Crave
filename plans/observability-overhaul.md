...
> **Correction 2026-08-03 (truth audit):** §3.1's "Existing dashboards are under
> `observability/grafana/provisioning/dashboards/`" is false against code as of
> today — there is **no `observability/` directory in the repo at all**, so §3.1's
> inventory/audit actions and §7's "remove Loki panels" actions have no subject.
> Likewise nothing in §2.3/§4 exists yet: repo-wide grep finds `METRICS_ACCESS_TOKEN`
> ONLY in this file; `apps/api/package.json` has no `prom-client`; there is no
> `@Controller('metrics')` anywhere in `apps/api/src`; and `yarn ops:report` is not a
> script in the root or api `package.json`. What IS real of §2.1 is Sentry only
> (`@sentry/nestjs` + `@sentry/profiling-node` in `apps/api/package.json:73-74`,
> `@sentry/react-native` in `apps/mobile/package.json:30`). Read this doc as an
> unbuilt plan, not a partially-landed one — nothing here has shipped except the
> Sentry dependency.
