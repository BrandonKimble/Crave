...
| POLL\_\* dead subset           | POLL_AGGREGATION_ENABLED, POLL_CITY_MIN_IMPRESSIONS, POLL_DEFAULT_COOLDOWN_DAYS, POLL_MAX_OPTIONS_PER_POLL, POLL_NOTIFICATIONS_ENABLED, POLL_TREND_COOLDOWN_DAYS, POLL_TREND_MIN_IMPRESSIONS | **CORRECTION 2026-08-04 (audit F727):** the note "remaining POLL_ vars ARE read (poll-scheduler.service.ts, poll-timing.ts)" is half-stale — `poll-scheduler.service.ts` does not exist (`PollScheduler` has zero hits in `apps/api/src`); only `poll-timing.ts` reads the remaining POLL_ vars.                                                                                                                 |
...
| KEYWORD*CITY*\*                | KEYWORD_CITY_DEMAND_WINDOW_DAYS, KEYWORD_CITY_MIN_IMPRESSIONS                                                                                                                                | **CORRECTION 2026-08-04 (audit F727):** "moved to collection_schedules rows (2026-07-08)" is stale — that table was itself deleted in a later consolidation (see F724, `plans/collection-scheduler-consolidation.md`); keyword cadence now lives on `source_collection_lanes`.                                                                                                                            |
