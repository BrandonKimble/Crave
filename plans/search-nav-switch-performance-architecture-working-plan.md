...
>
> **Correction 2026-08-03 (truth audit), file-specific:** the instrumentation surface it
> measures against is partly gone. Present today:
> `apps/mobile/src/screens/Search/runtime/shared/search-nav-switch-perf-probe.ts` and
> `apps/mobile/src/perf/js-frame-sampler.ts`. ABSENT: `apps/mobile/src/overlays/
SearchRouteMountedSceneOwners.tsx`, `apps/mobile/src/perf/harness-config.ts`, and
> `apps/mobile/src/screens/Search/runtime/telemetry/nav-switch-harness-observer.ts`. The
> `nav-switch-baseline-20260417a` / `-autonomous-20260417b` evidence files under `/tmp`
> are long gone, so none of the numbers below are re-verifiable.
