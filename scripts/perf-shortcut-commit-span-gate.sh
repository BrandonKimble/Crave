...
usage() {
  cat <<'USAGE'
Usage:
  scripts/perf-shortcut-commit-span-gate.sh \
...
Notes:
  - Commit-span is evaluated only for spans that intersect the run-1 worst JS frame window.
  - Policy:
      1) median candidate top heavy-component max commit span must improve vs control,
      2) no heavy component median max commit-span regression may exceed +10%.
USAGE
}
...
SUMMARY_PATH="/tmp/perf-shortcut-commit-span-gate-$(date -u +%Y%m%dT%H%M%SZ).json"
...
const hasIntersection = (leftStart, leftEnd, rightStart, rightEnd) =>
...
const parseRunOneCommitSpan = (logPath) => {
  ...
  const maxCommitSpanByComponent = Object.fromEntries(components.map((componentId) => [componentId, 0]));
  ...
  for (const span of runOneSpans) {
    ...
    if (span.commitSpanMs > maxCommitSpanByComponent[span.id]) {
      maxCommitSpanByComponent[span.id] = span.commitSpanMs;
    }
  ...
  const topMaxCommitSpanMs = components.reduce(
    (maxValue, componentId) =>
      Math.max(maxValue, maxCommitSpanByComponent[componentId] ?? 0),
    0
  );
...
const controls = controlLogPaths.map((logPath) => parseRunOneCommitSpan(logPath));
const candidates = candidateLogPaths.map((logPath) => parseRunOneCommitSpan(logPath));

const candidateMedianMaxCommitSpanByComponent = Object.fromEntries(
  components.map((componentId) => [
    componentId,
    median(candidates.map((entry) => entry.maxCommitSpanByComponent[componentId] ?? 0)),
  ])
);
const controlMedianMaxCommitSpanByComponent = Object.fromEntries(
  components.map((componentId) => [
    componentId,
    median(controls.map((entry) => entry.maxCommitSpanByComponent[componentId] ?? 0)),
  ])
);

const controlMedianTopMaxCommitSpanMs = median(controls.map((entry) => entry.topMaxCommitSpanMs));
const candidateMedianTopMaxCommitSpanMs = components.reduce(
  (maxValue, componentId) =>
    Math.max(maxValue, candidateMedianMaxCommitSpanByComponent[componentId] ?? 0),
  0
);
...
const componentDeltaPct = Object.fromEntries(
  components.map((componentId) => [
    componentId,
    deltaPct(
      controlMedianMaxCommitSpanByComponent[componentId] ?? 0,
      candidateMedianMaxCommitSpanByComponent[componentId] ?? 0
    ),
  ])
);
...
const directionalImprovement =
  controlMedianTopMaxCommitSpanMs != null &&
  candidateMedianTopMaxCommitSpanMs < controlMedianTopMaxCommitSpanMs - EPSILON;

const failures = [];
if (!directionalImprovement) {
  failures.push(
    `Median candidate top max commit span (${candidateMedianTopMaxCommitSpanMs.toFixed(
      3
    )}ms) must improve vs control (${String(controlMedianTopMaxCommitSpanMs)}ms).`
  );
}
if (regressedComponents.length > 0) {
  failures.push(
    `Component max-commit-span regression exceeds +${MAX_COMPONENT_REGRESSION_PCT}%: ${regressedComponents
...
const summary = {
  schemaVersion: 'perf-shortcut-commit-span-gate.v1',
  generatedAt: new Date().toISOString(),
  policy: {
    requiredDirectionalSignal: 'candidate median top max commit span improves vs control',
  ...
  candidates,
  medians: {
    controlMaxCommitSpanByComponent: controlMedianMaxCommitSpanByComponent,
    candidateMedianMaxCommitSpanByComponent,
    componentDeltaPct,
    controlTopMaxCommitSpanMs: controlMedianTopMaxCommitSpanMs,
    candidateMedianTopMaxCommitSpanMs,
    topDeltaPct: deltaPct(controlMedianTopMaxCommitSpanMs ?? 0, candidateMedianTopMaxCommitSpanMs),
  },
...
echo "[perf-commit-span-gate] Summary: $SUMMARY_PATH"
