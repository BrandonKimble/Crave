...
for target in "$ROOT_TARGET" "$COMPOSITION_TARGET" "$PRESENTATION_TARGET" "$HARNESS_TARGET" "$MAP_QUERY_TARGET" "$BOUNDS_SERVICE_TARGET"; do
  if [[ ! -f "$target" ]]; then
...
require_pattern "$ROOT_TARGET" "viewportBoundsService\\.setBounds\\(" \
  "Bounds updates are delegated to viewport bounds service."
