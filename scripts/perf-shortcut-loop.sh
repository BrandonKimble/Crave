...
if [[ -z "${EXPO_PUBLIC_PERF_HARNESS_RUNS:-}" ]]; then
  if [[ "$EXPO_PUBLIC_PERF_HARNESS_SCENARIO" == "search_shortcut_loop_open_now_roundtrip" ]]; then
    export EXPO_PUBLIC_PERF_HARNESS_RUNS="1"
  else
    export EXPO_PUBLIC_PERF_HARNESS_RUNS="3"
  fi
fi
...
export EXPO_PUBLIC_PERF_SHORTCUT_SETTLE_BOUNDARY_POLICY="${EXPO_PUBLIC_PERF_SHORTCUT_SETTLE_BOUNDARY_POLICY:-quiet_snapshot_only}"
...
echo "  settleBoundaryPolicy=${EXPO_PUBLIC_PERF_SHORTCUT_SETTLE_BOUNDARY_POLICY}"
...
if ! [[ "$EXPO_PUBLIC_PERF_HARNESS_COOLDOWN_MS" =~ ^[0-9]+$ ]]; then
  echo "Invalid EXPO_PUBLIC_PERF_HARNESS_COOLDOWN_MS value: ${EXPO_PUBLIC_PERF_HARNESS_COOLDOWN_MS}" >&2
  exit 1
fi
if [[ "$EXPO_PUBLIC_PERF_HARNESS_COOLDOWN_MS" -gt 5000 ]]; then
  echo "[perf-shortcut-loop] Clamping cooldown to 5000ms (requested ${EXPO_PUBLIC_PERF_HARNESS_COOLDOWN_MS}ms)." >&2
  EXPO_PUBLIC_PERF_HARNESS_COOLDOWN_MS=5000
fi
